...
class Cas14
    ...
    def estApplicable?(unNoeud)
        if( unNoeud.nbLiens == 4 && unNoeud.nbVoisinsUnLien() == 4 ) then
            return true
        else
            return false
        end
    ...
    def appliquer(unNoeud, niveau)
        if(niveau == 1) # Aide textuelle simple
            ...
            $aide.set_text "Un noeud de taille 4 qui possède 4 voisins ne pouvant créer qu'un lien peut compléter ses liens."
