...
class Cas10
    ...
    def estApplicable?(unNoeud)
        if( unNoeud.nbLiens == 5 && unNoeud.nbVoisinsZeroLien() == 2 ) then
            return true
        else
            return false
        end
    ...
    def appliquer(unNoeud, niveau)
        if(niveau == 1) # Aide textuelle simple
            ...
            $aide.set_text "Un noeud de taille 5 qui possède 2 voisins ne pouvant créer qu'un lien peut compléter une partie de ses liens."
